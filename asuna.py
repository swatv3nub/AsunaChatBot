...
@asuna.on_mesaage(filters.command("start"))
async def start(_, message):
    asunapic = "https://telegra.ph/file/cb3b9a091ee2480fc2cea.jpg"
    if message.chat.type == "private":
        message.reply_photo(photo=asunapic, caption=start_text, reply_markup=keyboard, parse_mode="markdown")
    else:
        message.reply_text("**Asuna [アスナ]** is Alive\nowo :3", parse_mode="markdown")

@asuna.on_callback_query(filters.regex("start_back"))
async def start_back(_, CallbackQuery):
    await asuna.send_message(
...
@asuna.on_message(~filters.edited & ~filters.command("start"))
